---
name: compile-export
description: Compile and sign a Jelly shortcut using the jelly CLI and shortcuts sign. Full pipeline with pre-flight checks.
---
...
```sh
make build FILE=<filename>.jelly
```
...
```sh
make deploy FILE=<filename>.jelly
```

This runs build → sign → open in sequence.
